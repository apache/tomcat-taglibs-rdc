...
<rdc:comment>
  Description: Collect, validate and confirm an ISBN.
  Attributes:
...
<c:choose>
  <c:when test="${empty stateMap[id]}">
    ...
    <jsp:useBean id="model" class="org.apache.taglibs.rdc.ISBN" >
      ...
      <jsp:useBean id="voice_grammar"
       class="org.apache.taglibs.rdc.core.Grammar" >
          <c:set target="${voice_grammar}" property="grammar"
           value="${pageContext.request.contextPath}/.grammar/isbn.grxml"/>
      </jsp:useBean>
      <c:set target="${model}" property="grammar" value="${voice_grammar}"/>
      <rdc:configure model="${model}" config="${config}" 
        defaultConfig="META-INF/tags/rdc/config/isbn.xml" />
