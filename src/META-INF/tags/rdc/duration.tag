...
<%@ attribute name="minDuration" required="false" %>
<%@ attribute name="maxDuration" required="false" %>
...
<rdc:comment>
  ...
  minDurationh: minimum allowed duration
...
<c:choose>
  <c:when test="${empty stateMap[id]}">
    ...
    <jsp:useBean id="model" class="org.apache.taglibs.rdc.Duration" >
      ...
      <c:set target ="${model}" property="maxDuration" value="${maxDuration}"/>
      <c:set target ="${model}" property="minDuration" value="${minDuration}"/>
      ...
      <c:set target="${model}" property="grammar"
       value="${pageContext.request.contextPath}/.grammar/duration.grxml"/>
      <rdc:configure model="${model}" config="${config}" 
        defaultConfig="META-INF/tags/rdc/config/duration.xml" />
