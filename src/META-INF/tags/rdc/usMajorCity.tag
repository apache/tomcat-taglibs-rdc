...
<rdc:comment>
  Description: Collect, validate and confirm a US major city.
  Attributes:
...
<c:choose>
  <c:when test="${empty stateMap[id]}">
    ...
    <jsp:useBean id="model" class="org.apache.taglibs.rdc.USMajorCity" >
      ...
      <c:set target="${model}" property="grammar"
       value="${pageContext.request.contextPath}/.grammar/usmajorcity.grxml"/>
      <rdc:configure model="${model}" config="${config}" 
       defaultConfig="META-INF/tags/rdc/config/usMajorCity.xml" />
